...
| `price_px_int` | Int64 | Price in fixed-point integer | `round(price / price_increment)` |
...
| `price_px_int` | Int64 | Limit price (lot-based, nullable) | `round(price / 0.01 / 100)` |
...
- Price: `price_px_int = round(price / 0.01 / 100)` (0.01 CNY per share, 100 shares/lot)
...
| `price_px_int` | Int64 | Execution price (lot-based, 0 for cancellations) | `round(price / 0.01 / 100)` |
...
**Decoding:**
```python
price = price_px_int * price_increment
...
price = 50123.45
price_px_int = round(50123.45 / 0.01) = 5012345  # Stored as Int64
...
**Encoding:**
```python
price_px_int = round(price / 0.01 / 100)  # CNY/share → lots
qty_int = round(qty / 100)  # shares → lots
```

**Decoding:**
```python
price = price_px_int * 0.01 * 100  # lots → CNY/share
...
price_px_int = round(15.23 / 0.01 / 100) = 15  # Stored as Int64
