"""SZSE L3 ticks ingestion service orchestrating the Bronze → Silver pipeline."""
...
from pointline.tables.szse_l3_ticks import (
    encode_fixed_point,
    normalize_szse_l3_ticks_schema,
    parse_quant360_ticks_csv,
    resolve_symbol_ids,
    validate_szse_l3_ticks,
)

logger = logging.getLogger(__name__)


class SzseL3TicksIngestionService(BaseService):
    """
    Orchestrates SZSE L3 tick ingestion from Bronze CSV files to Silver Delta tables.
    ...
    def __init__(
        ...
        """
        Initialize the SZSE L3 ticks ingestion service.

        Args:
            repo: Repository for the szse_l3_ticks Silver table
    ...
    def validate(self, data: pl.DataFrame) -> pl.DataFrame:
        """Validate SZSE L3 ticks data schema and quality."""
        return validate_szse_l3_ticks(data)

    def compute_state(self, valid_data: pl.DataFrame) -> pl.DataFrame:
        """
        Transform validated ticks data to final Silver format.
        ...
        return normalize_szse_l3_ticks_schema(valid_data)

    def write(self, result: pl.DataFrame) -> None:
        """Append ticks data to the Delta table."""
        if result.is_empty():
        ...
        if hasattr(self.repo, "append"):
            ...
            raise NotImplementedError("Repository must support append() for szse_l3_ticks")
    ...
    def ingest_file(
        ...
        *,
        ...
        """
        Ingest a single SZSE L3 ticks CSV file from Bronze to Silver.
        ...
        try:
            ...
            parsed_df = parse_quant360_ticks_csv(raw_df)
            ...
            # 9. Normalize schema
            normalized_df = normalize_szse_l3_ticks_schema(final_df)
            ...
            logger.info(
                f"Ingested {validated_df.height} ticks from {meta.bronze_file_path} "
