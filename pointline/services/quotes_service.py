"""Quotes ingestion service orchestrating the Bronze → Silver pipeline."""
...
class QuotesIngestionService(BaseService):
    """
    Orchestrates quotes ingestion from Bronze CSV files to Silver Delta tables.
    ...
    def __init__(
        ...
        """
        Initialize the quotes ingestion service.

        Args:
            repo: Repository for the quotes Silver table
    ...
    def validate(self, data: pl.DataFrame) -> pl.DataFrame:
        """Validate quotes data schema and quality."""
        return validate_quotes(data)

    def compute_state(self, valid_data: pl.DataFrame) -> pl.DataFrame:
        """
        Transform validated quotes data to final Silver format.
        ...
        return normalize_quotes_schema(valid_data)

    def write(self, result: pl.DataFrame) -> None:
        """Append quotes data to the Delta table."""
        if result.is_empty():
        ...
        if hasattr(self.repo, "append"):
            ...
            # For quotes, we could use a composite key, but append is preferred
            raise NotImplementedError("Repository must support append() for quotes")
    ...
    def ingest_file(
        ...
        *,
        ...
        """
        Ingest a single quotes CSV file from Bronze to Silver.
        ...
        try:
            ...
            # 9. Normalize schema
            normalized_df = normalize_quotes_schema(final_df)
            ...
            logger.info(
                f"Ingested {validated_df.height} quotes from {meta.bronze_file_path} "
    ...
    def validate_ingested(
        ...
        *,
        ...
        match_expr = pl.all_horizontal(
            [
                pl.col("bid_px_int") == pl.col("bid_px_int_ing"),
                pl.col("bid_sz_int") == pl.col("bid_sz_int_ing"),
                pl.col("ask_px_int") == pl.col("ask_px_int_ing"),
                pl.col("ask_sz_int") == pl.col("ask_sz_int_ing"),
            ]
        ).alias("_row_match")
        matched_by_line = (
