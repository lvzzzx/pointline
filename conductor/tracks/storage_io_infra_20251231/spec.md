...
- **Protocol Definition:** Create `src/io/protocols.py` to define the `TableRepository` protocol, ensuring all future repositories adhere to a standard interface (`read_all`, `write_full`, `merge`).
- **Base Repository Implementation:** Implement `BaseDeltaRepository` in `src/io/base_repository.py` (or `delta_repository.py`) to encapsulate common `delta-rs` and Polars I/O operations.
