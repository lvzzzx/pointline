...
- **Protocol Definition:** Create `src/io/protocols.py` to define the `TableRepository` protocol, ensuring all future repositories adhere to a standard interface (`read_all`, `write_full`, `append`, `merge`).
- **Base Repository Implementation:** Implement `BaseDeltaRepository` in `src/io/base_repository.py` (or `delta_repository.py`) to encapsulate common `delta-rs` and Polars I/O operations, including `overwrite`, `append`, and `merge` behaviors.
- **Centralized Configuration:** Create `src/config.py` to manage:
...
- **Directory Scaffolding:** Establish the `src/services/` and `src/io/` directories to support the new architectural layers.
...
- [ ] `src/config.py` is the single source of truth for paths and storage parameters.
