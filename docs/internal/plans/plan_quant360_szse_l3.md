...
**Key Difference:**
- **L2 (existing Tardis):** Aggregated by price level: `{price_px_int: 50000, size_int: 1000}` (all orders at this price)
- **L3 (SZSE):** Order-by-order: `{order_id: 12345, price_px_int: 50000, qty_int: 100}` (individual orders tracked)
...
SZSE_L3_ORDERS_SCHEMA = {
    ...
    "price_px_int": pl.Int64,              # fixed-point encoded
...
SZSE_L3_TICKS_SCHEMA = {
    ...
    "price_px_int": pl.Int64,              # 0 for cancellations
...
def validate_ticks(df: pl.DataFrame) -> pl.DataFrame:
    ...
    - For fill: price_px_int > 0
    - For cancel: price_px_int == 0 is okay
    - qty_int > 0
...
def reconstruct_l3_book(
    ...
    Returns:
        {
            "bids": [
                {"order_id": 123, "price_px_int": 50000, "qty_int": 100},
                ...
            ],
            "asks": [...],
        }
...
def l3_to_l2_aggregate(l3_book: dict) -> pl.DataFrame:
    ...
    Returns DataFrame with columns:
        side (0=bid, 1=ask), price_px_int, total_qty_int, order_count
    """
